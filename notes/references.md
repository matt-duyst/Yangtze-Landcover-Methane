...
`notes/references.bib` carries one hundred ninety-nine entries as BibTeX. It is
...
ninety-nine cited DOIs resolved: one hundred eighty-one through Crossref and
...
something broader. **The register holds nine preprints: seven on arXiv and two
as Copernicus discussion papers**, which the 14 September 2026 audit corrected
from "six on arXiv and three as Copernicus" — the total was right and the split
was not. That audit also found that **three of the nine do not say so in their
role line**, against the claim once made here that every one of them does:
`long2026urbanpreprint`, `montenegro2025capability` and `boulesteix2012plea`
carry no role label at all. And **three of the nine have since been published**
and are annotated as superseded where they sit: Nab and Groenwold in *Global
Epidemiology*, Boulesteix and others in *PLoS ONE*, and Lee and others in the
ACM SIGKDD proceedings. Two
...
## The three categories, and how each entry's metadata was established

Added 14 September 2026, after every DOI in the register was re-negotiated. Until
then this file held three kinds of DOI-shaped string and defined only one of
them, which is how the third came to be created as a one-off.

### The definitions, which a reader should not have to infer

**A normal entry** is a citation whose BibTeX was generated by content
negotiation. There are one hundred ninety-nine of them and they are the whole
of `notes/references.bib`.

**`EXCLUDED`** holds **real citations of this project that cannot be generated**.
Each is a source used for a real claim; what they share is that content
negotiation cannot produce an entry, so the generator's guarantee — every field
is what `https://doi.org` returned — cannot be met for them, and typing one by
hand would break that guarantee for the whole file. They are named in
`scripts/build_references_bib.py`, kept out of the BibTeX, and where they have a
DOI it is written **without backticks** so the generator does not claim it.

**`NOT_CITATIONS`** holds **strings that look like DOIs and are not citations at
all**. Each was carried into a pass as a real reference and turned out to resolve
to the wrong paper or to nothing. They are named so that a future reader meeting
one in a search result knows it was tested and rejected.

**The distinction that matters**: `EXCLUDED` is about a *mechanism* failing on a
good source; `NOT_CITATIONS` is about a *source* failing. An excluded entry is as
much a citation as any other and its claims are relied on normally. It is not a
backlog either — nothing in it is waiting to be moved, because nothing about
those sources will change. A future pass should revisit an excluded entry only if
its publisher begins honouring BibTeX negotiation, and should add to the set only
after an attempt has actually been made and failed. **A DOI that was never tried
is an unverified entry, not an excluded one.**

### The four excluded entries

| Entry | Why it cannot be generated | How its metadata was established |
|---|---|---|
| ISO 5807:1985 | a standard; no DOI exists | the ISO catalogue record |
| Chaudhuri (2020) | a textbook; no DOI exists | the book's title page |
| Zhang, W., and others (2026), *Scientia Agricultura Sinica* | the DOI resolves, but the publisher answers `Accept: application/x-bibtex` with HTML | the publisher's own English-language article page |
| *Atmospheric Chemistry and Physics* (2026), review criteria | a journal policy page rather than a work; there is no identifier to negotiate | the journal's own site, fetched on the negotiation date |

This table listed three until 14 September 2026, when an audit against the
generator found `EXCLUDED` holds four: the journal policy page was named in the
code and not in the prose. The *Scientia Agricultura Sinica* entry is the one
that forced this section. It is the register's first
citation that **has** an identifier and still cannot be generated, and it was
marked `EXCLUDED` beside the standard and the textbook before the category had a
definition that admitted it.

### The five warned-against strings

They are listed with their reasons under *What could not be verified* above, and
the generator holds the same reasons in `NOT_CITATIONS` so that none can acquire
a key by accident. Four resolve confidently to the wrong paper; the fifth has the
right article number and the wrong prefix and resolves to nothing.

### Provenance, recorded by exception rather than per entry

**The question this section answers is the one the 14 September audit had to
reconstruct from git history**: for any given entry, was its text taken from the
registry or from somewhere else? That could be recorded per entry, and it should
not be. Every entry but seventeen now has the same answer, so a per-entry field
would carry one value one hundred eighty times — 197 lines of file for 17
lines of information.

**So the convention is a stated default plus a marked exception list.** The
default: `scripts/build_references_bib.py` records `NEGOTIATED = "2026-09-14"`,
the date on which every DOI in this register was content-negotiated and its
author list, title, container, volume, pages and year compared against the prose.
Unless an entry appears in `PROVENANCE`, that is where its text came from.

**The seventeen exceptions are entries that negotiate but whose registry record
cannot supply the whole citation**, and `PROVENANCE` gives the reason for each:
two IPCC chapters for which Crossref returns no author list at all; eight
deposits and products whose author fields arrive as literal institutional
strings, transposed, mis-cased, or with given and family names run together; the
natural-gas-vehicle paper whose first author Crossref stores as the bare literal
"Da Pan"; a 1999 paper for which Crossref returns one author of four; and two
JSTOR records that carry a start page where the register gives the range.

**The cost is one dictionary of seventeen entries and this section.** The benefit
is that the next audit begins by reading a date and a list rather than by
replaying twelve commits, which is what this one had to do. And the exception
list is itself the answer to a question that took the audit most of its effort:
these seventeen are the entries a naive re-negotiation will flag every time, and
...
W. (2021).** High Resolution Distribution Dataset of Double-Season Paddy Rice in
China. *Remote Sensing* 13, article 4609.
`10.3390/rs13224609` — peer-reviewed paper; describes a dataset used, CC BY 4.0.
**This is the citation for the one column the anonymous route cannot supply.**
The National Ecosystem Science Data Center rasters the committed analysis grid
was built from carry a double-season class that the Science Data Bank export
folds into the background, and `rice_fraction_combined` is the column that
difference produces. Four authors are shared with Shen et al. above — Shen,
Ye, Dong and Yuan — so the two classes are companion products of one group
rather than independent datasets, which is what makes the FTP copy citable at
all. Found 14 September 2026, by searching for the double-season class rather
than for the product, after four passes had recorded the column as resting on
an uncitable source. Cited in `config/sources.yml`.
...
arXiv.
`10.48550/arXiv.2106.04285` — **preprint, SUPERSEDED: published as**
*Global Epidemiology* 3, 100067 (2021), `10.1016/j.gloepi.2021.100067`,
same two authors and same title, verified 14 September 2026. **A manuscript
must cite the journal version.** The preprint entry is kept because it is what
was consulted. Originally recorded as: preprint; a method not applied. The comparison
...
Domains. arXiv.
`10.48550/arXiv.2505.14312` — **preprint, SUPERSEDED: published as**
*Proceedings of the 32nd ACM SIGKDD Conference on Knowledge Discovery and Data
Mining*, 9278–9289 (2026), `10.1145/3770855.3817455`, verified 14 September
2026. A manuscript must cite the proceedings version. Originally recorded as:
preprint; **a finding that contests the convenient
...
estimate". **The threshold's own attribution does not hold**: this paper credits
it to Shen et al. (2022), which states no DOFS threshold anywhere in its text or
supplement and instead gives an emission-rate and observation-count criterion.
Checked 15 September 2026 against both papers. Quote this paper for the
threshold, since the words are its own, and do not present it as the field's
standard.
Cited in `notes/grounding-methods.md`.

**Shen, L., Gautam, R., Omara, M., Zavala-Araiza, D., Maasakkers, J. D.,
Scarpelli, T. R., Lorente, A., Lyon, D., Sheng, J., Varon, D. J., Nesser, H.,
Qu, Z., Lu, X., Sulprizio, M. P., Hamburg, S. P., and Jacob, D. J. (2022).**
Satellite quantification of oil and natural gas methane emissions in the US and
Canada including contributions from individual basins.
*Atmospheric Chemistry and Physics* 22, 11203–11215.
`10.5194/acp-22-11203-2022` — peer-reviewed paper; a finding relied on. **Added
because the entry above misattributes a threshold to it, and because what it
actually says is more useful than what it was said to say.** Its own criterion,
verbatim: TROPOMI observations "can quantify emission rates with an uncertainty
(2σ) better than 30 % in areas with emissions exceeding 0.2–0.5 Tg a⁻¹ and
observation counts exceeding 5000 a⁻¹". That is an emission-rate and
observation-count criterion rather than a DOFS one, this domain meets both with
room to spare, and it is the better-founded basis for the claim that this record
can constrain a domain total. DOFS appears once in the paper, defined and not
thresholded, and the supplement carries it only as a figure label.
Cited in `notes/grounding-methods.md`, `notes/draft-methods.md`.
...
`10.48550/arXiv.1208.2651` — **preprint, SUPERSEDED: published as**
*PLoS ONE* 8, e61562 (2013), `10.1371/journal.pone.0061562`, verified
14 September 2026. **The published version carries three authors where this
preprint entry carries two** — Boulesteix, Lauer and Eugster — which is the
truncated-author-list class this register guards against, arriving this time
through a preprint rather than a deposit. A manuscript must cite the journal
version and all three authors. Originally recorded as: **preprint, not
peer-reviewed**; a finding relied
