...
**Five deposits exist for entries that recorded none**, and all five were found
in the papers' own data availability or Data Records sections rather than by
searching: the 500 m irrigated cropland maps, China's oil and gas CH4
inventory, the underground wastewater plants, the 30 m building heights, and
CMAB. A sixth, the GHGSat plume set, was found the same way and is on Zenodo,
which is blocked. **The lesson is the one the CCD-Rice polygons already taught
and it was not applied: read the article's data statement, not the product's
landing page.** Four of these six entries were marked *documented only* on the
strength of having read the abstract.
